...
class Generator:
    ...
    def _lm_inputs(self, content: str):
        ...
        if input_ids.shape[1] == 0:
            ...
            if bos is None:
                raise ValueError(
                    "Empty prompt but no bos_token_id / pad_token_id / eos_token_id; "
                    "cannot prime the language model."
                )
    ...
    def apply_logit_penalty(self, log_probs, step, penalty=-5.0):
        """Sec. 4.1: logit penalty -5 on non-alphabetic tokens at the first iteration."""
        if step != 0:
            return log_probs
    ...
    def mask_non_ascii_tokens(self, log_probs):
        """Sec. 3.3: restrict candidate vocabulary to ASCII tokens."""
        out = log_probs.clone()
        for token_id in range(len(log_probs)):
            token = self.tokenizer.decode([token_id])
